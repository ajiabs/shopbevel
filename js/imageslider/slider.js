...
Slider.Gallery = Class.create();
Slider.Gallery.prototype = {
    images : [],
    file2id : {
        'no_selection' :0
    },
    ...
    handleUploadComplete : function(files) {
        files.each( function(item) {
            ...
            newImage.imageText='Please save added image before adding image text';
            newImage.disabled = 0;
            newImage.removed = 0;
    ...
    createImageRow : function(image) {
        ...
        $(vars.id).select('td[class="cell-image a-center"]').each(function(e) {
            e.style.display='none';
        }.bind(this));

        $(vars.id).select('td[class="cell-small_image a-center"]').each(function(e) {
            e.style.display='none';
        }.bind(this));
        $(vars.id).select('td[class="cell-thumbnail a-center"]').each(function(e) {
            e.style.display='none';
        }.bind(this));                
    },
    ...
    updateVisualisation : function(file) {
        ...
        this.getFileElement(file, 'cell-text textarea').value=image.imageText;
        $H(this.imageTypes)
                .each(
                        function(pair) {
...
Slider.Attributes = Class.create();
Slider.Attributes.prototype = {
    config : {},
    containerId :null,
    ...
    setConfig : function(config) {
        this.config = config;
        Slider.AttributesBridge.bind(this.getConfig().tab_id, this);
    },
    getConfig : function() {
        return this.config;
    },
...
Slider.Configurable = Class.create();
Slider.Configurable.prototype = {
    initialize : function(attributes, links, idPrefix, grid, readonly) {
        ...
        this.newSliders = []; // For product that's created through Create
        ...
        this.grid.checkboxCheckCallback = this.registerSlider.bind(this); // Associate/Unassociate
                                                                            // simple
                                                                            // product
    ...
    registerSlider : function(grid, element, checked) {
        if (checked) {
    ...
    updateSlider : function(productId, attributes) {
    ...
    quickCreateNewSlider : function() {
        ...
        new Ajax.Request(this.createQuickUrl, {
            ...
            onComplete :this.quickCreateNewSliderComplete.bind(this)
        });
    },
    quickCreateNewSliderComplete : function(transport) {
