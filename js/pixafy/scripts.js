...
var ENABLE_REGISTER_FLAG = false;
		     	...
		     	$j.post(
					...
					function(data)
					{
						...
						if(data.error == undefined) {
							...
							$j('#login-error').text(data.error).show();
						}
...
function showContestTool( str ) {
	...
	jQuery('.sign-in-box .overlay-signup-social').fadeIn(50);
        ...
        jQuery('#login-contesttool').val('1');
        jQuery('#contesttoolreg').val('1')
      
         //  jQuery('#login-contesttool').val("1");alert("ff");
       return false;
...
jQuery(document).ready(function(){
	...
	$j( '.product-slider' ).find( '.slider-item' ).columns({
		columns: 4
	});

	$j( '.product-slider' ).contentSlider({
		transition: 'slide',
		useControls: true,
		pauseOnHover: true,
		auto: false,
	...
	$j( '.trunkshow-items-box' ).find('.items-wrapper').columns({
		columns: 4,
		duration: 400
	});
	...
	$j( '.trunkshow-items-box' ).find( '.other-products-wrapper' ).columns({
		columns: 4,
		duration: 400
	});
	...
	$j( 'body' ).on('mouseenter', '.prdt-img', function() {
		showVoteItemShare.call( this );
	});

	$j( 'body' ).on('mouseleave', '.prdt-img', function() {
		hideVoteItemShare.call( this );
	});

	$j( 'body' ).on( 'click', '.vote-item-share', function( e ) {
		goToItemPage.call( this, e );
	});

	$j( '.designer-info' ).find( '.vote-dsgnr-desc' ).verticalAlign( 'center' );

	$j( '.shop-landing-grid-wrapper' ).columns({
		columns: 4,
		duration: 400
	});

	// chop the names of the home page mini slider at top to
	// prevent line breaks
	chopNames( $j( '.product-slider' )
				.find( '.vote-dsgnr-desc')
				.find( '.txt-dsgnr-name' ) );

	chopLocs( $j( '.product-slider' )
				.find( '.vote-dsgnr-desc')
				.find( '.txt-locale' ) );

}); /* end document.ready */

function chopNames( arr ) {
	$j.each( arr, function() {
		var self = $j( this );
		self.text( chopChars( 13, self.text() ) );
	});
}

function chopLocs( arr ) {
	$j.each( arr, function() {
		var self = $j( this );
		self.text( chopChars( 13, self.text() ) );
	});
...
/**
 * return the string with the character limit set
 * @param  {Integer} limit the amount of chars to return
 * @param  {String} str   the string to limit
 * @return {String}       the string parameter sliced
 */
function chopChars( limit, str ) {
	return str.slice( 0, limit );
}

/**
 * used on vote item modules to redirect user to
 * @param  {[type]} e [description]
 * @return {[type]}   [description]
 */
function goToItemPage( e ) {
	//check to make sure click target is not a share button,
	//if it is not then redirect to item page, else stay put
	if ( !$j(e.target).hasClass( 'share-link') ) {
		var self = $j( this );
		window.location = self.data( 'url' );
	}
}

function showVoteItemShare() {
	var self = $j( this );
	var shareContainer = self.children( '.vote-item-share' );
	shareContainer.stop().fadeIn( 300 );
}

function hideVoteItemShare() {
	var self = $j( this );
	var shareContainer = self.children( '.vote-item-share' );
	shareContainer.stop().fadeOut( 300 );
}
...
function togglePriceTooltip() {
	...
	if( tooltip.css( 'display' ) === 'block' || 'inline') {
...
function ajax_register_result(data)
{
	...
	else
	{
        ...
        if(data.hasOwnProperty('customer_id')) {
            $j(document).trigger('shopBevel_customerSignUp', data.customer_id);
        }
...
$j.fn.verticalAlign = function( str ) {
	...
	this.align = function() {
		switch ( str.toLowerCase() ) {
			...
			case 'center':
				this.css( 'margin-top', (this.space / 2) + 'px' );
			break;
...
function shareFb(elementObj)
{
	  ...
	  var designId = elementObj.data("design");
	  ...
	  function(response)
	  {
		if (response && response.post_id)
		{
			if(designId)
			{
				$j.post(getUrl(VOTE_FRONTNAME + "/index/share"), {'id':designId, 'shareType':'fb'});
			}
		}
		else
		{
