...
class Mage_Checkout_Block_Cart_Sidebar extends Mage_Checkout_Block_Cart_Abstract
{
	...
	//Check cart to see if time has expire on the cart
	function checkCart()
	{
		if($this->helper("checkout/cart")->getItemsCount())
		{
			$secsToday = strtotime(date('Y-m-d H:i:s'));
			
			//Parse Time when first item was added
			$firstItem = $this->helper("checkout/cart")->getFirstCartItem();
			if($firstItem)
			{
				$date = new DateTime($firstItem->getCreatedAt());
				
				//Add x amt of time available
				$date->add(new DateInterval('PT'.$this->helper('checkout/cart')->getCartTimeLimit().'M'));
				$secsExpire = strtotime($date->format('Y-m-d H:i:s'));
				
				//Compare to see if there is still a positive difference
				//Clear cart if no time left
				$diff = ($secsExpire - $secsToday)/60;
				if($diff <= 0)
				{
					$this->helper('checkout/cart')->clearCart();		
				}
			}
			else
			{
	...
	//functon to build item htm
	function ajaxGetItems()
	{
		$items = $this->getRecentItems();
		$html = "<table class='top-cart-table'>
            	<tbody>
				";
