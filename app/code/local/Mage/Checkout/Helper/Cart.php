...
class Mage_Checkout_Helper_Cart extends Mage_Core_Helper_Url
{
	...
	{
		$quote = Mage::getSingleton('checkout/session')->getQuote();
		$firstItem = current($quote->getAllVisibleItems());
		return $firstItem;
	}
	
	//Return time limit(in mins) for products in cart
	...
	{
		$timitLimit = $this->getCartTimeLimit();
		$firstProduct = $this->getFirstCartItem();
		if($firstProduct)
		{
			//Build date time object
			$date = new DateTime($firstProduct->getCreatedAt());
			//$date->add(new DateInterval('PT'.$timitLimit.'M'));		
