 ...
 */
class Pixafy_Bevel_AccountController extends Mage_Core_Controller_Front_Action {
    ...
    public function loginPostAction() {
        ...
        if ($this->_getSession()->isLoggedIn()) { 
            if ($this->getRequest()->isXmlHttpRequest()) {
                $json['redirect_url'] = Mage::getUrl('home');
       ...
       $session = $this->_getSession();
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ((!empty($login['username']) && !empty($login['password'])) || !empty($login['fb-id'])) {
                try {
                    if (!empty($login['fb-id'])) {
                        // Find if their FB id is present, if so sign them in
                        $customers = Mage::getModel('customer/customer')->getCollection()->addAttributeToFilter('fb_id', array('eq' => $login['fb-id']));
                        if (count($customers) == 1) {
                            foreach ($customers as $c) {
                                $session->setCustomerAsLoggedIn($c);
                            }
                        } else {
                            $message = "You must signup before logging in with Facebook";
                            $json['error'] = $message;
                        }
                    } else {
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            /* if(stripos($refererUrl, $checkoutPath))
              {
              $json['redirect_url'] = $refererUrl;
              }
              else
              {
              $json['redirect_url'] = Mage::getUrl('shop');
              } */
            ...
            if ($login['design-id']) {
                ...
                $rewriteUrl = $pixvoteHelper->getDesignRewriteUrl($login['design-id']);
                if ($rewriteUrl) {
    ...
    protected function _loginPostRedirect() {
        ...
        if (!$session->getBeforeAuthUrl() || $session->getBeforeAuthUrl() == Mage::getBaseUrl()) {
            ...
            if ($login['design-id']) {
                ...
                $rewriteUrl = $pixvoteHelper->getDesignRewriteUrl($login['design-id']);
                if ($rewriteUrl) {
                    $session->setBeforeAuthUrl($rewriteUrl);
                }
            } else if ($refererUrl != Mage::getBaseUrl()) {
                $session->setBeforeAuthUrl($refererUrl);
            } else {
                $session->setBeforeAuthUrl(Mage::getUrl('shop'));
            }
            ...
            if ($session->isLoggedIn()) {
                ...
                //CHANGE TO HOMEPAGE
                $session->setBeforeAuthUrl(Mage::getUrl('home'));
    ...
    public function createPostAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $customerForm->setFormCode('customer_account_create')
                    ->setEntity($customer);
            //print_r($this->getRequest()->getPost('contest_tool_flag'));exit;
            ...
            try {
                ...
                if (true === $validationResult) {
                    ...
                    if ($customer->isConfirmationRequired()) {
                        ...
                        if ($this->getRequest()->getPost('is-designer')) {
                            $customer->setIsDesigner($this->getRequest()->getPost('is-designer'));
                            $customer->save();
                        }
                        ...
                        if ($this->getRequest()->isXmlHttpRequest()) {
                            $json['success'] = true;
                            $json['customer_id'] = $customer->getId();

                            if ($this->getRequest()->getPost('redirect-designer')) {
                                $json['redirect_designer_url'] = Mage::getUrl("submit-about");
                            } else {
                                $refererUrl = $this->_getRefererUrl();
                                $json['redirect_url'] = $refererUrl == Mage::getBaseUrl() ? Mage::getUrl('shop') : $refererUrl;
                                if ($this->getRequest()->getPost('design-id')) {
                                    //redirect to design submission
                                    $pixvoteHelper = Mage::helper("pixvote");
                                    $rewriteUrl = $pixvoteHelper->getDesignRewriteUrl($this->getRequest()->getPost('design-id'));
                                    if ($rewriteUrl) {
                                        $json['redirect_url'] = $rewriteUrl;
                                    }
                                }
                            ...
                            if ($this->getRequest()->getPost('contesttoolreg') == 1) {
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($this->getRequest()->getPost('login-contest_tool_flag') == 1) {
